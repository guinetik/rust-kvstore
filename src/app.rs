...
impl App {
    ...
    fn read(&self, key: String) {
        ...
        if value != "" {
            self.print_keypair_formatted(&key, value, formatting);
        } else {
    ...
    fn print_help(&self) {
        ...
        logger.display(format!(""));
        logger.display(format!(
            "gui-kvstore --print --debug=true|false --f=default|csv|json|short --store=STORE_NAME"
        ));
        ...
        match formatting.as_str() {
            "short" => {
                for (_key, value) in db {
                    self.logger.display(format!("{}", value));
                }
            }
            "csv" => {
                let headers = "key,value";
            ...
            _ => {
                for (key, value) in db {
                    self.logger.display(format!("{}={}", key, value));
                }
            }
        }
    }
